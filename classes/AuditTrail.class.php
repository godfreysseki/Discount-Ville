<?php
  ...
  class AuditTrail extends Config
  {
    ...
    public function logActivity($userId, $activityType, $entityId, $activity, $details = '', $oldValue = '', $newValue = '', $module = '', $userAgent = '', $status = '')
    {
      
      $userAgent = $this->getBrowser() . " on " . $this->getOS();
      $status    = 'Success';
      $sql       = 'insert into audittrail (user_id, activity_type, entity_id, activity, details, old_value, new_value, module, user_agent, status) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)';
      $params    = [
        $userId,
        $activityType,
        $entityId,
    ...
    {
      $sql         = "select * from audittrail order by log_id desc";
